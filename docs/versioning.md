...
| 결과 JSON schema | `3.0` | 구조화된 issues와 경량 결과 정책을 포함한 JSON 형식 식별 |
...
결과 schema `1.1`, `2.0`과 구형 `check_arm_crossing` 또는 `output` 블록이 있는
Config는 자동 변환하지 않습니다. 현재 UI와 주문 생성 worker는 schema `3.0` 결과만
