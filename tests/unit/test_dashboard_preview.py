...
from waam_validator.constants import MODE_D, MODE_T
from waam_validator.dashboard.preview import _gantt_figure, _statistics_figures
...
def test_input_gantt_uses_one_duration_accurate_bar_per_robot() -> None:
    trajectories = TrajectorySet(
        (_trajectory(1, 3_000), _trajectory(2, 3_000), _trajectory(3, 3_000)),
        9_003,
    )

    figure = _gantt_figure(trajectories)
    ...
    assert float(figure.layout.xaxis.range[1]) < 3_000.0
    ...
    for robot_label in ("R1", "R2", "R3"):
        ...
        assert deposition_duration == 1_500.0
        assert travel_duration == 1_500.0
...
def test_input_time_percentages_and_reach_have_separate_axes(fixture_root: Path) -> None:
    ...
    for robot_index in range(3):
        ...
        assert all(str(trace.text[robot_index]).endswith("%") for trace in time_figure.data)
...
def test_time_chart_does_not_mislabel_post_completion_as_wait(fixture_root: Path) -> None:
    ...
    assert float(traces["Wait"].y[0]) == 0.0
    assert float(traces["완료 후 비활성"].y[0]) == 50.0

    gantt = _gantt_figure(trajectories)
